...
{
playindex=3;         //index = load which song
printf("%d\r\n",playindex);
wait(0.1);
queue.call(loadSong);
...
void mode_selection(){
  ...
  kk=fuck();    // kk=0 for foward song , kk=1 for backward song
  if(kk==0){
...
int main(void)
{
 ...
 // button.rise(queue.event(loadSongHandler));   // button 1 load song 1

 // button3.rise(queue.event(loadSongHandler1)); // button 2 load song 2


  while(1)
  {
      if(button==0){mode_selection();}
